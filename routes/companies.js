...
router.get("/", async function (req, res, next) {
  ...
  if(nameLike) {
    nameLike = `LOWER(name) LIKE '%${nameLike.toLowerCase()}%'`
    arr.push(nameLike)
  }
  if(minEmployees) {
    if(parseInt(minEmployees) > parseInt(maxEmployees)){
      return res.status(400).json("minEmployees cannot be greater than maxEmployees")
    }
     minEmployees = `num_employees >= ${minEmployees}`
     arr.push(minEmployees)
  }
  if(maxEmployees) {
    maxEmployees = `num_employees <= ${maxEmployees}`
    arr.push(maxEmployees)
  }
  ...
  try {
    ...
    const listOfCompanies = await Company.filterCompanies(arr)
